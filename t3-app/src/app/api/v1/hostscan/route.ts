    ...
    case "main":
      try {
        const resMain = await fetch(
          `${env.NEXT_PUBLIC_NETWORK_MAIN_URL}/v1/host/scan/?publicKey=${publicKey}`,
        );
        if (!resMain.ok) {
        ...
        const main = (await resMain.json()) as HostScan;
        // console.log(main);
        // if (main.length === 0) {
        //   return Response.json([], {
        //     status: 500,
    ...
    case "zen":
      try {
        const resZen = await fetch(
          `${env.NEXT_PUBLIC_NETWORK_ZEN_URL}/v1/host/scan/?publicKey=${publicKey}`,
        );
        console.log(resZen.statusText);
        if (!resZen.ok) {
          return Response.json(
            {},
            {
